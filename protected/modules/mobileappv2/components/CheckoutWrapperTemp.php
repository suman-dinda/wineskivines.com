...
{	
			...
			if ($json = json_decode($restrict_order_by_status,true)){
				...
				if(!empty($order_stats)){
					...
					if($res = Yii::app()->db->createCommand($stmt)->queryRow()){						
						throw new Exception( Yii::t("default","You have previous orders with status [status], you cannot place another order until your last order is process",array(
						  '[status]'=>t($res['status'])
						)) );
					}
				}
	...
	public static function verifyLocation($merchant_id=0, $lat=0, $lng=0,$order_subtotal=0)
	{
        ...
        if($resp){
        	...
        	if($merchant_delivery_distance>0){
	        	$resp_distance = MapsWrapperTemp::getDistance($resp['latitude'],$resp['lontitude'],$lat,$lng,$unit,$mode);
	        	...
	        	if($merchant_delivery_distance>0){
		        	if($distance>$merchant_delivery_distance){	        		
	        		   $pretty_distance = Yii::t("default","[distance] [unit]",array(
			    					  ...
			    					  '[unit]'=>MapsWrapperTemp::prettyUnit($unit)
			    					));
	...
	*/
	...
	{											
		...
		$resp_distance = MapsWrapperTemp::getDistance($data['from_lat'],$data['from_lng'],$data['to_lat'],$data['to_lng'],$unit,$mode);
		if($resp_distance){			
			$distance = $resp_distance['distance'];			
